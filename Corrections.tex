...
% 		\end{subfigure}
% 	}%
% 	\caption[Results of TPC and TOF single-track efficiency correction closure test on embedded signal MC sample (using nominal eff.).]{Results of TPC and TOF single-track efficiency correction closure test on embedded signal MC sample. Distributions of the true-level track $p_{\text{T}}$~\subref{fig:tpcTofSingleTrkClosureTrackPt} and true-level $\eta$~\subref{fig:tpcTofSingleTrkClosureTrackEta} corrected for single-particle TPC efficiency (red points) and single-particle TPC and TOF efficiency (blue points) are compared with the true-level distributions of corresponding observables (black points). }\label{fig:closureTestTpcTofSingleTrk_SingleParticleEmbeddingCorrection}%
% \end{figure}
% %---------------------------
