...
\begin{enumerate}
 ...
 \epsilon_{\text{RP}}^{\text{side}} = \mbox{\LARGE$\varepsilon$}\left(\RPSIDE\left|\TRSIDE\land~!\TRNSIDE\right.\right) = %
 \frac{(p_{x}, p_{y})~\text{histogram for protons from}~set~B}{(p_{x}, p_{y})~\text{histogram for protons from}~set~A}
  \end{equation}
