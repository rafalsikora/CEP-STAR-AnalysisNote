...
The last step involved proper normalization of the histograms obtained earlier. Predictions for each true-level pair type were normalized independently, but the same normalization was preserved for all quantities and for all reconstructed pair types. For example, normalization of histograms representing prediction for exclusive $\pi^{+}\pi^{-}$ was obtained by ...

\begin{equation}
 F_{\pi\pi}^{\text{MC}} = \frac{\left(1-f_{bkgd}^{non-excl}\right)\int\limits_{0}^{12}\frac{dN_{\pi\pi}^{\text{data}}}{d\chi^{2}_{dE/dx}}d\chi^{2}_{dE/dx}}{\int\limits_{0}^{12}\left(\frac{dN_{\pi\pi\rightarrow\pi\pi}^{\text{MC}}}{d\chi^{2}_{dE/dx}} + \frac{dN_{KK\rightarrow\pi\pi}^{\text{MC}}}{d\chi^{2}_{dE/dx}} + \frac{dN_{pp\rightarrow\pi\pi}^{\text{MC}}}{d\chi^{2}_{dE/dx}}\right)d\chi^{2}_{dE/dx} }
\end{equation}
